...
export default function Students() {
  ...
  async function deleteStudent() {
    ...
    if (error) {
      ...
      setToast({ message: 'Estudiante eliminado' })
      setSelected(null)
      loadStudents()
    }
  }
  ...
  async function saveGuardian() {
    ...
    if (editingGuardianId) {
      ...
      await supabase
        .from('student_guardians')
        .update({ relationship: guardianRelationship, is_primary_contact: guardianIsPrimary })
        .eq('student_id', selected.id)
        .eq('guardian_id', editingGuardianId)
    } else {
      ...
      const { error: linkError } = await supabase
        ...
        .insert({
          ...
          is_primary_contact: guardianIsPrimary,
        })
      if (linkError) {
        setToast({ message: `Error vinculando: ${linkError.message}`, type: 'error' })
        setSavingGuardian(false)
        return
      }
    }
  ...
  async function removeGuardian(linkId) {
    ...
    if (error) {
      ...
      setToast({ message: 'Acudiente removido' })
      openDetail(selected)
    }
  }
  ...
  function openEditStudent(student) {
    setForm({
      ...
      nationality: student.nationality || 'Dominicana',
  ...
  function openEditGuardian(link) {
    ...
    setGuardianRelationship(link.relationship || 'other')
    setGuardianIsPrimary(link.is_primary_contact || false)
    ...
    <div className="animate-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button className="btn btn-primary" onClick={openAddStudent} style={{ height: 40 }}>
            ...
            <tbody>
              {loading ? (
                ...
                >
                  ...
                  <td style={{ padding: '12px 16px', color: 'var(--text-secondary)' }} onClick={() => openDetail(s)}>
                    {s.date_of_birth ? new Date(s.date_of_birth + 'T00:00:00').toLocaleDateString('es-DO') : '—'}
                  </td>
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', justifyContent: 'flex-end' }}>
          ...
          <div style={{
            position: 'relative', width: '100%', maxWidth: 460, background: 'var(--bg-card)',
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button className="btn btn-ghost btn-sm" onClick={() => openEditStudent(selected)} title="Editar">
                  <Pencil size={16} />
                </button>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <button className="btn btn-secondary btn-sm" onClick={openAddGuardian}>
              ...
              {guardians.length === 0 ? (
                ...
                <div key={i} className="card" style={{ padding: 14, marginBottom: 8, background: 'var(--bg-secondary)', position: 'relative' }}>
                    ...
                    <div>
                      ...
                      {g.guardians?.cedula && (
                        <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 2 }}>
                          Cédula: {g.guardians.cedula}
                        </div>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: 2 }}>
                      <button className="btn btn-ghost btn-sm" onClick={() => openEditGuardian(g)} style={{ padding: 4 }} title="Editar">
                        <Pencil size={14} />
                      </button>
                      <button className="btn btn-ghost btn-sm" onClick={() => removeGuardian(g.id)} style={{ padding: 4, color: 'var(--danger)' }} title="Remover">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
              ...
              <div style={{ marginTop: 32, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
                  {Object.entries(STATUS_LABELS).map(([key, label]) => (
                    <button
                      key={key}
                      className={`btn btn-sm ${selected.enrollment_status === key ? 'btn-primary' : 'btn-secondary'}`}
                      style={{ fontSize: 12 }}
                      disabled={selected.enrollment_status === key}
                      onClick={async () => {
                        const { error } = await supabase.from('students').update({ enrollment_status: key }).eq('id', selected.id)
                        if (!error) {
                          setToast({ message: `Estado cambiado a ${label}` })
                          setSelected({ ...selected, enrollment_status: key })
                          loadStudents()
                        }
                      }}
                    >
                ...
                {!deleteConfirm ? (
                  <button className="btn btn-sm" onClick={() => setDeleteConfirm(true)}
                    style={{ background: 'transparent', color: 'var(--danger)', border: '1px solid var(--danger)', fontSize: 12 }}>
                  ...
                  <div style={{ padding: 14, background: 'var(--danger-light)', borderRadius: 'var(--radius-md)', border: '1px solid var(--danger)' }}>
                    <p style={{ fontSize: 13, color: 'var(--danger)', fontWeight: 500, marginBottom: 10 }}>
                      ¿Estás seguro? Esta acción ocultará al estudiante de todas las listas.
                    </p>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn btn-danger btn-sm" onClick={deleteStudent} disabled={deleting}>
      ...
      <Modal
        ...
        title={editMode ? 'Editar Estudiante' : 'Agregar Estudiante'}
        width={620}
      >
        <div style={{ padding: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <Field label="Código de Estudiante">
              <input style={inputStyle} value={form.student_code}
            ...
            <Field label="Grado Actual">
              <select style={selectStyle} value={form.current_grade_level_id}
                onChange={e => setForm({ ...form, current_grade_level_id: e.target.value })}>
            ...
            <Field label="Estado">
              <select style={selectStyle} value={form.enrollment_status}
            ...
            <Field label="Notas Médicas" span={2}>
              <textarea style={{ ...inputStyle, minHeight: 70, resize: 'vertical' }} value={form.medical_notes}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
            ...
            <button className="btn btn-primary" onClick={saveStudent}
              ...
              <Save size={16} />
              {saving ? 'Guardando...' : editMode ? 'Actualizar' : 'Agregar Estudiante'}
            </button>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title={editingGuardianId ? 'Editar Acudiente' : 'Agregar Acudiente'}
        width={580}
      >
        <div style={{ padding: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <Field label="Nombre" required>
              <input style={inputStyle} value={guardianForm.first_name}
                onChange={e => setGuardianForm({ ...guardianForm, first_name: e.target.value })}
                placeholder="Ej: María" />
            </Field>
            <Field label="Apellido" required>
              <input style={inputStyle} value={guardianForm.last_name}
                onChange={e => setGuardianForm({ ...guardianForm, last_name: e.target.value })}
                placeholder="Ej: González" />
            </Field>
            <Field label="Parentesco" required>
              <select style={selectStyle} value={guardianRelationship}
            ...
            <Field label="Cédula">
              <input style={inputStyle} value={guardianForm.cedula}
                onChange={e => setGuardianForm({ ...guardianForm, cedula: e.target.value })}
                placeholder="000-0000000-0" />
            </Field>
            <Field label="Teléfono Principal" required>
              <input style={inputStyle} value={guardianForm.phone_primary}
                onChange={e => setGuardianForm({ ...guardianForm, phone_primary: e.target.value })}
                placeholder="809-000-0000" />
            </Field>
            <Field label="Teléfono Secundario">
              <input style={inputStyle} value={guardianForm.phone_secondary}
                onChange={e => setGuardianForm({ ...guardianForm, phone_secondary: e.target.value })}
                placeholder="809-000-0000" />
            </Field>
            <Field label="Email" span={2}>
              <input style={inputStyle} type="email" value={guardianForm.email}
                onChange={e => setGuardianForm({ ...guardianForm, email: e.target.value })}
                placeholder="correo@ejemplo.com" />
            </Field>
            <Field label="Dirección" span={2}>
              <input style={inputStyle} value={guardianForm.address}
                onChange={e => setGuardianForm({ ...guardianForm, address: e.target.value })}
                placeholder="Calle, número..." />
            </Field>
            <Field label="Ciudad">
              <input style={inputStyle} value={guardianForm.city}
                onChange={e => setGuardianForm({ ...guardianForm, city: e.target.value })} />
            </Field>
            <Field label="Provincia">
              <input style={inputStyle} value={guardianForm.province}
                onChange={e => setGuardianForm({ ...guardianForm, province: e.target.value })} />
            </Field>
            <Field label="Ocupación">
              <input style={inputStyle} value={guardianForm.occupation}
                onChange={e => setGuardianForm({ ...guardianForm, occupation: e.target.value })}
                placeholder="Ej: Ingeniero" />
            </Field>
            <Field label="Lugar de Trabajo">
              <input style={inputStyle} value={guardianForm.workplace}
                onChange={e => setGuardianForm({ ...guardianForm, workplace: e.target.value })} />
            </Field>

            {/* Primary contact toggle */}
            <div style={{ gridColumn: 'span 2', display: 'flex', alignItems: 'center', gap: 10, paddingTop: 8 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14 }}>
                <input type="checkbox" checked={guardianIsPrimary}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
            ...
            <button className="btn btn-primary" onClick={saveGuardian}
              disabled={savingGuardian || !guardianForm.first_name || !guardianForm.last_name || !guardianForm.phone_primary}>
              <Save size={16} />
              {savingGuardian ? 'Guardando...' : editingGuardianId ? 'Actualizar' : 'Agregar Acudiente'}
            </button>
          </div>
        </div>
      </Modal>
